...
(function() {
  ...
  window.widgetFunctions.resetTransformation = function(instanceId) {
    ...
    // Restore original comparison layout if carousel was injected
    const resultsState = getElement(instanceId, 'resultsState');
    if (resultsState) {
      const originalComparison = resultsState.querySelector('.integrated-results');
      if (originalComparison) originalComparison.style.display = '';
      const carousel = resultsState.querySelector('.gvc-root');
  ...
  // ============================================================
  // VARIANT SELECTION + CAROUSEL MODULE
  // ============================================================

  // Fetch configured variants for this product (storefront-safe endpoint)
  ...
  // After photo is captured/selected: check for configured variants, then route accordingly
  async function checkVariantsAndProceed(instanceId, file) {
    ...
    if (variants.length === 0) {
      // No configured variants — existing single-transform flow
      transformImage(instanceId, file);
      return;
    }

    // Open variant selector (auto-proceeds if only 1 variant)
  ...
  // Send multi-variant request (variantIds[] array) and return slides
  ...
  // Show results inline (in resultsState div) — used by integrated widget
  function showCarouselInline(instanceId, slides) {
    if (slides.length === 1) {
      // Single result: use original display path
      const beforeImg = getElement(instanceId, 'beforeImage');
      const afterImg = getElement(instanceId, 'afterImage');
    ...
    // Hide the original before/after comparison, inject carousel above it
    const originalComparison = resultsState.querySelector('.integrated-results');
    if (originalComparison) originalComparison.style.display = 'none';

    // Remove any prior carousel
  ...
  // Build the enterprise tab carousel into a container element
  ...
  function openVariantModal(variants, onConfirm) {
    // Auto-proceed without showing modal when only 1 variant
